...
export default function App() {
  ...
  async function transitionToMajor() {
    ...
    if (arpPatRef.current)  { arpPatRef.current.stop(); arpPatRef.current.dispose(); arpPatRef.current = null }
    if (!isMuted) {
      filtRef.current?.frequency.rampTo(8000, 1.5)
      bassRef.current?.triggerAttackRelease('A2', '2n')
      setTimeout(() => { padRef.current?.triggerAttackRelease(['A3','C#4','E4'], '2n') }, 400)
      setTimeout(() => { padRef.current?.triggerAttackRelease(['A4','C#5','E5'], '2n') }, 900)
      setTimeout(() => {
        padRef.current?.triggerAttackRelease(['A3','C#4','E4','A4','C#5','E5'], '1m')
        bellRef.current?.triggerAttackRelease('8n')
      }, 1500)
      setTimeout(() => bellRef.current?.triggerAttackRelease('8n'), 2000)
      setTimeout(() => bellRef.current?.triggerAttackRelease('8n'), 2400)
      setTimeout(() => bellRef.current?.triggerAttackRelease('8n'), 2700)
      setTimeout(() => bellRef.current?.triggerAttackRelease('8n'), 2900)
